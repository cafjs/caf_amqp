...
exports.newInstance = function($, spec, cb) {
    try {
        ...
        var login = (typeof spec.env.login === 'string' ? spec.env.login :
                     null);
        var password = (typeof spec.env.password === 'string' ?
                        spec.env.password : null);
        var authMechanism = (typeof spec.env.authMechanism === 'string' ?
                             spec.env.authMechanism : null);
                        ...
                        unsubscribeAll(function(err) {
                            if (err) {
                                ...
                                try {
                                    ...
                                    $._.$.log &&
                                        $._.$.log.debug('Error closing AMQP:' +
                                                        myUtils
                                                        .errToPrettyStr(ex));
                                }
                                con = null;
        ...
        con = amqp.createConnection(options, {
            reconnect: true // rabbitmq specific
        });

        con.on('ready', function () {
            exc = con.exchange(undefined, {confirm: true}, function (e) {
                cb(null, that);
            });
        });
        ...
        con.on('error', function(err) {
            $._.$.log && $._.$.log.error('Lost AMQP connection:' +
                                         err);
            that.__ca_shutdown__(null, myUtils.callJustOnce(function(error) {
                if (error) {
                    $._.$.log && $._.$.log.error('Error in shutdown:' +
